#!/usr/bin/env python3
"""
EC2 リソース設定機能付き AWS Batch ジョブ送信スクリプト
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="EC2 リソース設定付き AWS Batch ジョブ送信ツール"
    )
    parser.add_argument(
        "--job-queue",
    ...
    parser.add_argument(
        "--vcpus",
        type=int,
        default=config.DEFAULT_RESOURCES["ec2"]["vcpu"],
        help="EC2 タスクに割り当てる vCPU 数",
    )
    parser.add_argument(
        "--memory",
        type=int,
        default=config.DEFAULT_RESOURCES["ec2"]["memory"],
        help="EC2 タスクに割り当てるメモリの MB 数",
    )
...
def main():
    ...
    job_name = f"ec2-resource-job-{timestamp}-{job_id_suffix}"
    ...
    logger.info(f"リソース設定: vCPU={args.vcpus}, メモリ={args.memory}MB")
