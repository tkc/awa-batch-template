#----------------------------------------------------------------------
# AWS Batch Job Failure Notification Setup
#----------------------------------------------------------------------

# Create SNS Topic (notification destination)
resource "aws_sns_topic" "batch_job_failure" {
  name = "${local.name_prefix}-batch-job-failure-fargate"
...
resource "aws_iam_role" "lambda_slack_role" {
  name = "${local.name_prefix}-lambda-slack-role-fargate"
...
resource "aws_cloudwatch_log_group" "lambda_logs" {
  name              = "/aws/lambda/${local.name_prefix}-slack-notifier-fargate"
...
resource "aws_lambda_function" "slack_notifier" {
  function_name    = "${local.name_prefix}-slack-notifier-fargate"
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "${local.name_prefix}-slack-notifier-fargate"
...
resource "aws_cloudwatch_log_metric_filter" "batch_job_failure" {
  name           = "${local.name_prefix}-batch-job-failure-filter-fargate"
...
resource "aws_cloudwatch_metric_alarm" "batch_job_failure" {
  alarm_name          = "${local.name_prefix}-batch-job-failure-alarm-fargate"
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "${local.name_prefix}-batch-job-failure-alarm-fargate"
